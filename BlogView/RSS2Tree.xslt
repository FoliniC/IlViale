  ...
  <xsl:template match="/atom:feed">
    <FeedHierarchy>
      <xsl:for-each select="atom:entry[count(. | key('year', substring(atom:published, 0, 5))[1]) = 1]">
        ...
        <year><xsl:attribute name="name"><xsl:value-of select="substring(atom:published, 0, 5)" />&lt;span>(<xsl:value-of select="count(key('year', substring(atom:published, 0, 5)))"/>)&lt;/span></xsl:attribute>
          <xsl:for-each select="key('year', substring(atom:published, 0, 5))[count(. | key('month', substring(atom:published, 0, 8))[1]) = 1]">
            ...
            <month><xsl:attribute name="name">
                  <xsl:call-template name="decode">
                  <xsl:with-param name="id">
                    <xsl:value-of select="substring(atom:published, 6, 2)" />
                  </xsl:with-param>
                  <xsl:with-param name="locale">it</xsl:with-param>
                </xsl:call-template>&lt;span>(<xsl:value-of select="count(key('month', substring(atom:published, 0, 8)))"/>)&lt;/span></xsl:attribute>
              <xsl:for-each select="key('month', substring(atom:published, 0, 8))[count(. | key('day', substring(atom:published, 0, 11))[1]) = 1]">
                ...
                <day><xsl:attribute name="name"><xsl:value-of select="substring(atom:published, 9, 2)" />&lt;span>(<xsl:value-of select="count(key('day', substring(atom:published, 0, 11)))"/>)&lt;/span></xsl:attribute>
